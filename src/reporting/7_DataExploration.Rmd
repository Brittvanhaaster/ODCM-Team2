...
This definitive data set of the historical data contains 31,342 observations covering the period from January 2023 to November 2025. Average queue times range from 5 (min) to 141 (max) minutes, with 5,290 missing values. Crowd levels and weather variables have a varied distribution. Note that this dataset also includes the feature engineered variables (starting from weekday_weekend).

```{r, echo = FALSE}
unique(efteling_his_complete$ride)
```

In order to verify the seed selection, the table above lists all the attractions of the Efteling. These are the same as in the live data.

\newpage

```{r, echo=FALSE}
efteling_his_complete %>%
distinct(ride, indoor_outdoor) %>%
ggplot(aes(x = indoor_outdoor)) +
geom_bar(fill = "steelblue") +
labs(
title = "Count of Indoor vs Outdoor Attractions",
x = "Type",
y = "Frequency"
) +
theme_minimal()

table(efteling_his_complete$indoor_outdoor)
```

The dataset appears to have nearly twice as many outdoor attractions, around the 20, as indoor attractions, around the 11. This reflects the park’s layout, where outdoor rides cover a big part of the overall parkmap. As a consequence, weather conditions are likely to influence the operational performance and waiting times of a big share of the rides.

```{r, echo=FALSE}
efteling_his_complete %>%
distinct(date, crowd_label) %>%
ggplot(aes(x = crowd_label)) +
geom_bar(fill = "darkorange") +
labs(
title = "Frequency of Crowd Levels",
x = "Crowd Category",
y = "Count"
) +
theme_minimal()

table(efteling_his_complete$crowd_label)
```

A distribution of the crowd labels show that the “Busy”, “Empty”, and “Quiet” crowd levels occur frequently, with between the 275 and the 320 observations per label. The “packed” conditions occur slightly less frequently. This shows overall, that extreme crowding conditions, such as packed and busy, is relatively less common, than moderate crowding, such as empty and quiet.

```{r, echo=FALSE}
efteling_his_complete %>%
  distinct(date, weekday_weekend) %>%
  ggplot(aes(x = weekday_weekend)) +
  geom_bar(fill = 'steelblue')+
  labs(
    title = "Frequency of Weekday vs Weekend",
    x = "Type of Day",
    y = "Count"
  ) + 
  theme_minimal()

table(efteling_his_complete$weekday_weekend)
```

Most observations come from weekdays. This distribution is quite similar to what is expected with the amount of week- and weekenddays is a week.

```{r, echo=FALSE}
efteling_his_complete %>%
  distinct(date, is_school_holiday_any) %>%
  ggplot(aes(x = is_school_holiday_any)) +
  geom_bar(fill = "blueviolet") +
  labs(
    title = "Frequency of School Holidays",
    x = "School Holiday",
    y = "Number of Days"
  ) +
  theme_minimal()

table(efteling_his_complete$is_school_holiday_any)
```

Most observations come from days where there was no school holiday. School holidays are overrepresented in this dataset, as in reality they do not account for one-third of all days.

```{r, echo=FALSE}
efteling_his_complete %>%
  distinct(date, is_holiday) %>%
  ggplot(aes(x = is_holiday)) +
  geom_bar(fill = "darkcyan") +
  labs(
    title = "Frequency of Public Holidays",
    x = "Public Holiday",
    y = "Number of Days"
  ) +
  theme_minimal()

table(efteling_his_complete$is_holiday)
```

The number of days with a public holiday are only a small part of the total amount of observations. This distribution is similar to real life, since there are only a few public holidays in a year.
...
```{r, echo = FALSE}
unique(efteling_live_queue$attraction_name)
```

In order to verify the seed selection, the table above lists all the attractions of the Efteling. These are the same as in the historical data.

```{r, echo=FALSE}
efteling_live_queue %>%
ggplot(aes(x = queue_type)) +
geom_bar(fill = "seagreen") +
labs(
title = "Frequency of Queue Types in Live Data",
x = "Queue Type",
y = "Number of Observations"
) +
theme_minimal()

table(efteling_live_queue$queue_type)
```

Most of the observations correspond to the standby queues, with over 6,000 recorded entries. The single-rider queues are less represented with less than 2,000 recorded entries. The live queue data primarily captured the standard queue behavior.

```{r, echo=FALSE}
efteling_live_queue %>%
  group_by(attraction_name) %>%
  summarise(mean_wait = mean(wait_time, na.rm = TRUE)) %>%
  arrange(desc(mean_wait)) %>%
  head(10)
```

This overview indicated that Danse Macabre, the steam trains, and Symbolica have the highest average waiting times, where the means all lay above the 23 minutes. Baron 1898, Joris en de Draak, and Python, the classic thrill rides, experience lower mean waiting times, generally between the 12 and 17 minutes. The overall distribution indicates a variation in attraction popularity.
...
```{r fig-daily-queue-raw, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r fig-daily-queue-7day, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r fig-monthly-rain, echo=FALSE, warning=FALSE, message=FALSE}
#Monthly rainfall
efteling_his_complete %>%
  mutate(
    date = as_date(date),
...
```{r fig-monthly-queue-vs-crowd, echo=FALSE, warning=FALSE, message=FALSE}
...
efteling_live_queue %>%
  ...
  labs(
    title = "Queue Patterns Throughout the Day for All Attractions",
    x = "Hour of Day",
    y = "Wait Time (minutes)"
  ) +
  facet_wrap(~ attraction_name, scales = "free_y") +
  theme_minimal()
```

This graph shows the queue development over the course of the day for some attractions. Below, some attractions are highlighted for better inspection. Based on this, researchers can inspect time-based popularity for rides. In the first example, the daily pattern for Danse Macabre indicates that the morning period consistently experiences the highest crowd pressure for this attraction. More graphs are presented now
...
#Queue pattern identification for one particular ride
efteling_live_queue %>%
  filter(attraction_name == "Vogel Rok") %>%
  filter(queue_type == "STANDBY") %>%
  mutate(hour = hour(timestamp) + minute(timestamp)/60) %>%
  ggplot(aes(x = hour, y = wait_time)) +
  geom_smooth(method = "loess", span = 0.3, se = TRUE, color = "blue", fill = "lightblue") +
  scale_x_continuous(breaks = 8:22, limits = c(8, 22)) +
  labs(
    title = "Queue Pattern for Vogel Rok Throughout the Day",
    x = "Hour of Day",
    y = "Wait Time (minutes)"
  ) +
  theme_minimal()
...
#Queue pattern identification for one particular ride
efteling_live_queue %>%
  filter(attraction_name == "Symbolica") %>%
  filter(queue_type == "STANDBY") %>%
  mutate(hour = hour(timestamp) + minute(timestamp)/60) %>%
  ggplot(aes(x = hour, y = wait_time)) +
  geom_smooth(method = "loess", span = 0.3, se = TRUE, color = "blue", fill = "lightblue") +
  scale_x_continuous(breaks = 8:22, limits = c(8, 22)) +
  labs(
    title = "Queue Pattern for Symbolica Throughout the Day",
    x = "Hour of Day",
    y = "Wait Time (minutes)"
  ) +
  theme_minimal()
...
#Queue pattern identification for one particular ride
efteling_live_queue %>%
  filter(attraction_name == "Fata Morgana") %>%
  filter(queue_type == "STANDBY") %>%
  mutate(hour = hour(timestamp) + minute(timestamp)/60) %>%
  ggplot(aes(x = hour, y = wait_time)) +
  geom_smooth(method = "loess", span = 0.3, se = TRUE, color = "blue", fill = "lightblue") +
  scale_x_continuous(breaks = 8:22, limits = c(8, 22)) +
  labs(
    title = "Queue Pattern for Fata Morgana Throughout the Day",
    x = "Hour of Day",
    y = "Wait Time (minutes)"
  ) +
  theme_minimal()
